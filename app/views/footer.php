<link rel="stylesheet" href="../../public/css/footer.css">
<footer class="bg-primary text-white">
    <div class="container p-6" style= "background: linear-gradient(to bottom, #594635, #88764c);" >
        <div class="row">
            <!-- Colonne de gauche: Logo et description -->
            <div class="col-md-4 text-center">
                <img src="../../public/images/Logo.png" alt="Votre Logo" width="100" class="mb-4">
                <p>ComicsFan.com est la destination ultime pour les passionnés de bandes dessinées...</p>
            </div>

            <!-- Colonne du milieu: Quizz, Formulaire -->
            <div class="col-md-4 text-center">
                <h5>Participez à nos Quizz</h5>
            </div>

            <!-- Colonne de droite: Liens utiles -->
            <div class="col-md-4 text-center">
                <h5>Liens utiles</h5>
                <a href="#" class="text-reset d-block">FAQ</a>
                <a href="#" class="text-reset d-block">Nous contacter</a>
                <!-- Autres liens utiles ici -->
            </div>
        </div>

        <div class="text-center p-3">
            © <?php echo date("Y"); ?> Tous droits réservés: <a class="text-white" href="https://votre-site-web.com/">ComicsFan.com</a>
        </div>
    </div>
</footer>